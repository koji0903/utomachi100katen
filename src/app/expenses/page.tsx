        ...
        <div className="p-4 sm:p-8 max-w-7xl mx-auto pb-24 lg:pb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">支出・経費管理</h1>
                        <p className="text-slate-500 text-sm">備品や消耗品の購入、領収書のAI解析・管理ができます。</p>
                    </div>
                </div>
                    ...
                    <div className="flex-1 flex gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all text-sm font-medium"
                            />
                        </div>
                        <input 
                            type="month"
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
                        ...
                        {(['すべて', '備品', '消耗品', '飲食費', '交通費', '通信費', '光熱費', '広告宣伝費', '支払手数料', 'その他'] as const).map(cat => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold transition-all border whitespace-nowrap ${filterCategory === cat ? 'bg-rose-600 text-white border-rose-600 shadow-md shadow-rose-100' : 'bg-white text-slate-500 border-slate-200 hover:border-rose-300'}`}
                            >
                                {cat}
                            </button>
                        ))}
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                <th className="px-6 py-4">内容 / 購入先</th>
                                ...
                                <tr key={expense.id} className="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div>
                                            ...
                                            <div className="text-[10px] text-slate-400 font-medium italic mt-0.5">{expense.vendor || '-'}</div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        {expense.isConfirmed ? (
                                            <span className="text-[10px] font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">確認済</span>
                                        ) : (
                                            <span className="text-[10px] font-bold text-amber-500 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 flex items-center justify-center gap-1">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            {expense.fileUrl && (
                                                <button 
                                                    onClick={() => window.open(expense.fileUrl, '_blank')}
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <p className="text-slate-400 font-bold">この期間の支出記録はありません</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
